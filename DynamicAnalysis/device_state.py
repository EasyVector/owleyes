...
from .cnn_cam3 import get_result
import config
...
class DeviceState(object):
    ...
    def __init__(self, device, views, foreground_activity, activity_stack, background_services,
                 tag=None, screenshot_path=None, cam = None, gb_cam=None, gb=None):
        self.device = device
    ...
    def save2dir(self, output_dir=None):
        try:
            ...
            web_path = "static/images/latest/screen_%s.jpg" % self.tag
            # dest_gb_path = "%s/screen_%sgb.jpg" % (output_dir, self.tag)
            web_gb_cam_path = "static/images/latest/screen_%sgb_cam.jpg" % self.tag
            # dest_cam_path = "%s/screen_%scam.jpg" % (output_dir, self.tag)
            if self.device.adapters[self.device.minicap]:
                dest_screenshot_path = "%s/screen_%s.jpg" % (output_dir, self.tag)
                get_result(self.screenshot_path, config.output_dir, "jpg")
            else:
                dest_screenshot_path = "%s/screen_%s.png" % (output_dir, self.tag)
                get_result(self.screenshot_path, config.output_dir, "png")
